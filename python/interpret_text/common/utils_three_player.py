...
class ModelArguments():
    """default parameters used to initialize the model independent of module component types
    """
...
class BertPreprocessor():
    """A class that tokenizes and otherwise processes text to be encoded by a BERT model.
    """
    def __init__(self, tokenizer = None, max_length = 50, pad_to_max = True):
        """Initialize the BertPreprocessor.
        
        :param tokenizer: an initialized tokenizer with 'encode_plus' and 'convert ids to tokens' methods;
            defaults to None
        :type tokenizer: tokenizer, optional
        :param max_length: the max number of tokens allowable in an example, defaults to 50
        :type max_length: int, optional
        :param pad_to_max: whether to pad all examples to the same (max) length, defaults to True
...
class GlovePreprocessor:
    ...
    def __init__(self, text, count_thresh, token_cutoff):
        ...
        :type text: list
        :param count_thresh: the number of times a word has to appear in a sentence to be 
