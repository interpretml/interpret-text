...
# Assuming that the models are located in './models' directory
...
# Model selection
selected_model = st.sidebar.selectbox('Select a model', list(MODEL_LIST.keys()))
...
top_p = st.sidebar.slider('Top P', min_value=0.0, max_value=1.0, value=0.96, step=0.01)
...
st.markdown("### TODO: fix the new row writing issue. should not react. fix the data.append issue.")
...
st.write("Should you run into any interesting examples, please feel free to fill the form below and add it as a new row :)")

prompt = st.text_area('Prompt', value=data.iloc[st.session_state['index']]['prompt'], height=50)
completion = model._sample_api_single(prompt, temperature=temperature, max_tokens=max_tokens, top_p=top_p)

st.markdown(f'<div class="highlight"><span style="color:blue;">{selected_model}:</span></span><em>{completion}</em></div>', unsafe_allow_html=True)

with st.form("Add new row"):
    new_row = {col: [st.text_input(f"Enter new value for {col}")] for col in data.columns if col != 'prompt'}
