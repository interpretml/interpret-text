...
def start_frontend():
    st.set_page_config(page_title="Hierarchically Explaining LLM completions", layout="wide")
...
def explanation_text(parts, attribution_scores):
    ...
    for feat_idx in range(len(parts)):
        ...
        if np.isclose(coef, 0):
            explanation_text.append(f'<span style="color:#000000;font-size:16px;border-radius:2%;">{ngram_text}</span>')
        
        else:
            explanation_text.append(f'<span style="background-color:{color};color:#000000;font-size:16px;border-radius:2%;">{ngram_text} ({coef:.2f})</span>')
...
class ProgressBar:
    ...
    def __next__(self):
        try:
            val = next(self._iter)
            self.progress += 1
            self.bar.progress(self.progress / self.total, text=f"Running perturbations ({self.progress}/{self.total})")
            return val
