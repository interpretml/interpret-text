...
def extract_non_overlapping_ngrams(text, n):
    ...
    non_overlapping_ngrams = [tokens[i:i+n] for i in range(0, len(tokens), n)]
    # Convert each group of words into a tuple (to resemble the output of the nltk.ngrams function)
    non_overlapping_ngrams = [" ".join(group) for group in non_overlapping_ngrams]
