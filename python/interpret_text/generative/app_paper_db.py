    ...
    if submit_button:
        ...
        for k in tqdm(range(1)):
            ...
            for l in range(data["attention_weights"].shape[0]):
                ...
                token_contribs_per_head = np.linalg.norm((data["attention_weights"][l][:, gs, :, np.newaxis]*proj_contribs[:, :, :]), axis=-1)
                ...
                for t in range(3, n_prompt):
                    token_label = model_wrapped.tokenizer.decode(full_prompt_tokens[t])
                    next_token_label = model_wrapped.tokenizer.decode(full_prompt_tokens[t+1])
                    if token_label in ["\n", "Assistant", "Ass"]:
                        break
                    metadata = {"layer": l, "token_idx": t, "data_idx": k, "label": token_label}
                    flow_data.append({
        ...
        def plot_attention_trace(result, savepdf=None, title=None):
            ...
            ax.set_title(f"Attention from Tokens to the Generation")
        ...
        for m_idx, metric in enumerate(flowdf.type.unique()):
            ...
            for d in flowdf.data_idx.unique():
                ...
                labels = [df_sample[df_sample.token_idx == t]["label"].unique()[0] for t in df_sample.token_idx.unique() if t>0]
                fig = plot_attention_trace({"scores":df_pivot,
                                   "labels": labels}, title=metric, savepdf=f"./figures/tools_attention_{metric}.pdf")
                with cols[m_idx]:
                    st.pyplot(fig)
