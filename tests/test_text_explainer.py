...
from common_utils import create_binary_newsgroups_data, create_random_forest_tfidf, \
    create_random_forest_vectorizer, create_linear_vectorizer, create_reviews_data
...
class TestTextExplainer(object):
    ...
    def test_validate_text_explanation(self):
        ...
        ordered_local_importance_values = text_explanation.ordered_local_importance_values
        ...
        for c in range(len(tree_explanation)):
            ...
            vocab_intersect = np.array(list(set(tree_vocab.keys()).intersection(set(imp_pc))))
            tree_vocab_indexes = [tree_vocab[vocab_word] for vocab_word in vocab_intersect]
            text_vocab_indexes = [imp_pc.index(vocab_word) for vocab_word in vocab_intersect]
            ...
            text_imp = np.array(ordered_local_importance_values[c])[0, text_vocab_indexes].argsort()[..., ::-1]
